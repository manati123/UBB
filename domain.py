...
class Environment:
    ...
    def set_surface(self, rows, columns, surface):
        self.__rows = rows
        self.__columns = columns
    ...
    def generate_random_map(self, fill=0.2):
        for i in range(self.__rows):
            for j in range(self.__columns):
                if random() <= fill:
...
class DroneMap:
    ...
    def mark_detected_walls(self, readings, x, y):
        ...
        for direction in DIRECTIONS:
            x2, y2 = x + direction[0], y + direction[
                1]  # for each direction possible mark the squares until we hit a wall

            while 0 <= x < self.__rows and 0 <= y2 < self.__columns and x - readings[UP] <= x2 <= x + readings[
                DOWN] and y - readings[LEFT] <= y2 <= y + readings[RIGHT]:
